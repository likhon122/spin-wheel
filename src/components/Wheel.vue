...
function redrawWheel(canvas, angle, prizes) {
  ...
  for (let i = 0; i < prizes.length; ++i) {
    ...
    // Draw borders (both start and end of each segment)
    ctx.lineWidth = 10; // Border thickness
    ctx.strokeStyle = '#8482db'; // Border color

    // Draw border at the start of the segment
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + r * Math.cos(arcAngle1), cy + r * Math.sin(arcAngle1));
    ctx.stroke();

    // Draw border at the end of the segment
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + r * Math.cos(arcAngle2), cy + r * Math.sin(arcAngle2));
    ctx.stroke();
  }
}
...
function redrawFrame(canvas) {
  ...
  ctx.beginPath();
  ctx.arc(cx, cy, r * 1.012, 0, 2 * Math.PI, true);
  ctx.arc(cx, cy, r * 0.98, 0, 2 * Math.PI, false);
  ctx.fillStyle = '#8482db';
  ctx.fill();
  ...
  ctx.fillStyle = '#8482db';
  ctx.beginPath();
  ctx.arc(cx, cy, r / 9, 0, 2 * Math.PI, false);
  ctx.fill();
